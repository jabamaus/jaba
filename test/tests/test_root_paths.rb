...
jtest 'checks src_root is valid' do
  sr = "#{temp_dir}/src_root"
...
jtest 'defaults build_root to cwd' do
  sr = "#{temp_dir}/src_root"
  FileUtils.mkdir(sr)
  IO.write("#{sr}/test.jaba", "")
  op = nil
...
jtest 'supports specifying src_root and build_root in jaba input' do
...
jtest 'supports specifying src_root and build_root on cmd line' do
  each_src_root_build_root do |sr, br|
    op = JABA.run(want_exceptions: true) do |c|
      c.src_root = sr
      c.build_root = br
