...
module JABA
  ...
  class TestText < JabaTest
    ...
    TEXT_JDL_FILE = "#{__dir__}/../../lib/jaba/modules/text/text.jdl.rb".cleanpath
    ...
    it 'fails if no filename specified' do
      check_fail "'t::filename' attribute requires a value. See #{TEXT_JDL_FILE}:#{find_line_number(TEXT_JDL_FILE, 'attr :filename')}", line: [__FILE__, 'tagY'] do
        jaba do
          text :t # tagY
        end
      end
