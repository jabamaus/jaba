...
class TestBoolAttribute < JabaTest
  ...
  it 'requires default to be true or false' do
    assert_jaba_error "Error at #{src_loc(__FILE__, :tagP)}: ':b' attribute default invalid: '1' is a integer - expected [true|false]" do
      jaba(barebones: true) do
        type :test do
          attr :b, type: :bool do
            default 1 # tagP
  ...
  it 'only allows boolean values' do
    assert_jaba_error "Error at #{src_loc(__FILE__, :tagW)}: 'b.c' attribute invalid: '1' is a integer - expected [true|false]" do
      jaba(barebones: true) do
        type :test do
          attr :c, type: :bool do
            default true
